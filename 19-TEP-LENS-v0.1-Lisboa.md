...
We present a geometric blind-prediction residual test for potential-dependent temporal propagation using SN Refsdal — the only multiply-imaged supernova with five resolved images and precision time delays. Seven independent lens-modelling teams predicted the SX reappearance delay *before* the measurement existed; all six non-zero residuals between their predictions and the subsequently observed delay are positive, matching the sign predicted by a log-magnification proxy model for temporal shear. The pre-specified Wilcoxon signed-rank test yields $p = 0.016$ ($2.2\sigma$; blind-only) and $p = 0.0078$ ($2.4\sigma$; all eight models). Signal-energy analysis confirms this is a single-contrast measurement: 99.9% of the predicted signal resides in the S4–SX baseline, with effective dimensionality $D_{\rm eff} \approx 2.0$. The measured coupling ($\alpha_{\rm lens} = -0.055 \pm 0.044$) is calibrated from the same data and thus definitional; the probative content lies entirely in the sign consistency across independent modelling codes. The test is insensitive to the Mass Sheet Degeneracy but remains limited by lens-model uncertainties ($\pm$16–60 d). Hierarchical Bayesian comparison is inconclusive (BF $\approx 1$), consistent with the current precision regime. Reducing model uncertainties to $\sigma < 5$ d would push this test to $>5\sigma$. Pre-registered falsification thresholds are provided for SN 2025wny.

Keywords: gravitational lensing, time delays, Temporal Equivalence Principle, SN Refsdal, blind prediction, Wilcoxon signed-rank test
...
Because the physically relevant quantity is expected to be closer to convergence than to magnification, a Monte Carlo sensitivity analysis (§2.4.2) that inverts the lensing identity $\kappa = 1 - \sqrt{\mu^{-1} + \gamma^2}$ over physically motivated shear priors and an unknown absolute magnification scale factor $C$. The inferred convergence values (median and 16th–84th percentiles) are:
...
#### 3.3.2 Signal-Energy Concentration and Effective Degrees of Freedom
...
#### 4.3.1 Lens-Model Bias Quantification

What correlated lens-model bias would be required to produce the observed 14.5 d residual without TEP? The observed SX delay is $\Delta t_{\rm SX,S1} = 376.0 \pm 5.6$ d (Kelly et al. 2023), and the weighted-mean blind prediction is $\Delta t_{\rm pred} = 361.4$ d. The residual is therefore $\mathcal{R}_{\rm obs} = +14.6$ d, requiring a systematic underprediction of $\sim$3.9% of the total SX delay across all seven independent modelling teams. Published lens-model comparisons for MACS J1149 (Treu et al. 2016) show inter-model scatter of $\sim$10–20% but no evidence for a universal directional bias at the 4% level. The TDCOSMO collaboration's systematic comparison of parametric and free-form codes on controlled mock clusters (Millon et al. 2020) found biases of $\lesssim$2% in time-delay predictions when the correct mass model is assumed, growing to $\sim$5–10% under model misspecification. A correlated 4% underprediction would therefore require a systematic bias at the upper end of known lens-model systematics — plausible but not generic. This quantification constrains the conventional explanation: if such a bias exists, it must be coherent across five independent modelling codes and specific to the SX–S1 delay direction.

### 4.4 The Observed vs. Blind-Predicted Test: What It Shows

The observed weighted-mean residual $\mathcal{R}_{\rm obs} = +14.6 \pm 11.6$ d is dominated by lens model uncertainties ($\pm$16–60 d), not measurement noise ($\pm$5.6 d). Three properties argue for a physical origin:

- **Sign consistency across methods.** Six of seven blind teams underestimate the delay using five independent codes (GLAFIC, LTM, WSLAP+, GLEE, LENSTOOL); the seventh (Diego) predicts exactly the observed value. No known conventional modelling bias explains the uniform direction.

- **Magnitude mapping is definitional.** Because $\alpha_{\rm lens}$ is defined as $\mathcal{R}_{\rm obs}/S$ (where $S$ is the unit geometric sensitivity), the apparent agreement $\alpha_{\rm inferred} \equiv \alpha_{\rm lens}$ is mathematically guaranteed. The probative content lies in sign consistency and scatter reduction, not the numerical coincidence.

- **Proxy-model correction reduces scatter.** Subtracting the predicted residual reduces the weighted RMS by 46% and brings 5 of 8 models into better agreement — the opposite of what GR predicts.

Reducing lens model uncertainties from $\pm$20–60 d to $\sim$5 d would push the binomial test to $p < 10^{-3}$ and make this test decisive. The Grillo et al. (2024) precision model ($\sigma = 16$ d) already represents a 4$\times$ improvement over the original blind models.

### 4.5 Robustness Stress Tests and Bayesian Priors

Three stress tests (detailed in §3.6.5) assess the sensitivity of the evidence to statistical and systematic assumptions:

- **Model dependence:** Leave-one-out exclusions across all eight models preserve the positive sign pattern in every realization.

- **Microlensing nuisance:** Monte Carlo flux-proxy perturbations (10–30%) leave the SX-loop residual centred near the nominal value with stable sign.

- **Hierarchical Bayesian comparison:** Non-decisive Bayes factors (BF $\approx 1$ for fixed-$\alpha$; BF $\approx 0.6$ for free-$\alpha$ with a GR-centred prior) confirm that current data are insufficient for decisive model selection.

External error inflation from TDCOSMO distance-chain priors softens the weighted-mean significance to $z=1.25$ while preserving the directional fit preference ($\Delta\chi^2=+1.56$). The implication is a reliability claim, not a detection claim: directional evidence persists across perturbations, while decisive model selection awaits tighter lens-model uncertainties.
...
| **Signal-energy concentration** §3.3.2 | Structural information-theoretic | Two SX loops contain 99.9% of predicted signal energy; S4–SX knockout removes 99.5%. Effective DOF $D_{\rm eff} = 2.0$. | Information-theoretic (no $\sigma$) | ✓ Yes (structural) | ✓ Structural |
| **Proxy-agnostic rank test** §3.4 | Rank correlation (delays vs potential depth) | Delay–kappa Spearman $\rho = -0.15$ ($p = 0.63$); delay–mu $\rho = -0.30$ ($p = 0.74$). Non-significant; confirms physical and parametric claims are not separately constrained by $n=5$ data. | Not meaningful as formal test | ✗ No (same SX data) | ✓ Observed (reported for transparency) |
...
No individual strand is decisive. Because several strands are correlated through the same SX-dominated data, combining p-values via meta-analysis is explicitly avoided. The headline remains the pre-specified blind-only Wilcoxon ($p = 0.016$, $2.2\sigma$), supported by the consistency of the other metrics. The break-even inter-model correlation for the Wilcoxon to soften to $p > 0.05$ is $\rho \approx 0.08$ (§3.6.5).

This paper claims strong directional evidence — not a detection. Lens model uncertainties ($\pm$16–60 d) prevent a $>3\sigma$ conclusion; precision models targeting $\sigma < 5$ d would make the test decisive.
...
A geometric blind-prediction residual test for potential-dependent temporal propagation has been applied to SN Refsdal, the only lensed supernova with five resolved images and precision time delays. The key results are:

- Under GR, all five algebraic loop sums are identically zero by construction. The genuine TEP observable is a blind-prediction residual — the discrepancy between observed delays and GR model predictions — not a closure violation.

- The inner Einstein cross is a **null region** where the $\mu \rightarrow \kappa$ proxy fails (shear degeneracy; rank-order agreement $P \approx 3$%). All probative content comes from the S4–SX contrast.

- The two SX loops yield predicted residuals of $-8.5$ d (SNR = 66) and $-14.5$ d (SNR = 63), far exceeding the 5.6 d measurement uncertainty. Signal-energy analysis confirms $D_{\rm eff} \approx 2.0$.

- The loop sum is insensitive to the Mass Sheet Degeneracy (a uniform convergence sheet rescales all delays by the same factor), but the empirical residual remains lens-model-limited.

- The pre-specified blind-only Wilcoxon signed-rank test gives $p = 0.016$ ($2.2\sigma$); the all-eight-model test gives $p = 0.0078$ ($2.4\sigma$). Robustness checks (LOO, microlensing MC, hierarchical Bayes with GR-centred prior) preserve the directional pattern. Bayesian model comparison is non-decisive (BF $\approx 1$), consistent with the current precision regime.

- **Internal consistency check — $H_0$.** The proxy-model-corrected SN Refsdal $H_0$ shifts from 66.6 to 69.3 km s$^{-1}$ Mpc$^{-1}$ — definitional, not independent, since $\alpha_{\rm lens}$ was calibrated from the same data.

- If independent delay measurements yield $|\mathcal{R}_{\rm obs}(\mathrm{S1, S4, SX})| < 1$ d, the log-magnification ansatz is falsified. Conversely, a residual consistent with $-14.5$ d would constitute direct geometric evidence once lens-model uncertainties are reduced to $\sigma < 5$ d.

- SN 2025wny ($z_s = 2.011$) provides the next falsifiable target: predicted GR discrepancies of 1–15 d at $\alpha_{\rm lens} \approx -0.055$, with pre-registered thresholds (§4.9).

The current evidence is suggestive but not decisive. The limiting factor is lens-model precision ($\pm$16–60 d), not measurement noise ($\pm$5.6 d) or signal amplitude (14.5 d). Reducing model uncertainties to $\sigma < 5$ d — achievable with extended-source modelling and next-generation mass reconstructions — would push this test to $>5\sigma$. This is a well-defined, falsifiable, purely geometric test whose power scales with model precision: the path from suggestive to decisive is entirely observational.
