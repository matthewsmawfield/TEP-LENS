#!/usr/bin/env python3
"""
TEP-LENS: Step 04 - Plot Predicted GR Discrepancies (SN Refsdal)

Generates two publication-quality figures:
  Figure 1: Per-loop comparison of GR null (0) vs TEP predicted GR discrepancy,
            with 1-sigma measurement sensitivity shown as error shading.
  Figure 2: TEP predicted discrepancy magnitude vs loop time-baseline, showing the
...
def main():
    print_status(f"STEP {STEP_NUM}: Generating Predicted GR Discrepancy Figures — SN Refsdal", "TITLE")
    ...
    R_tep = [loops[n]["tep_gr_discrepancy_days"] for n in loop_names]
    R_err = [loops[n]["tep_gr_discrepancy_err_days"] for n in loop_names]
    ...
    ax.set_xlabel('Algebraic Loop (image triplet)', )

    # fig.tight_layout()
    out_path1 = out_dir / f"step_{STEP_NUM}_gr_discrepancy.png"
