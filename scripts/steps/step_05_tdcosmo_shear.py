...
to a single underlying potential. Under TEP, the differential temporal shear
between image pairs of different magnification introduces a systematic offset
in the delay ratios relative to the GR lens model prediction.
...
If TEP is operative, images traversing deeper potential (higher magnification)
should arrive relatively later than GR predicts, producing a systematic
trend: rho increases with magnification contrast.
...
For each system we use flux ratios from HST imaging as magnification proxies.
"""
...
def tep_predicted_delay_ratio(dt_obs, flux_i, flux_A, alpha=-0.05):
    """
    ...
    Here we compute the TEP-predicted fractional shift in dt_iA relative
...
def main():
    ...
    print_status("Note: by construction rho=+1 since TEP_shift = alpha*log(F)*|dt|")
    ...
    results = {
        ...
        "summary": {
            ...
            "interpretation": (
                "TEP predicts systematic delay shifts of 0.1-10 days for TDCOSMO "
                "quad lenses. The fractional shift alpha*log10(F_i/F_A) is small "
                "for quasar systems (flux ratios near unity) compared to the "
                "SN Refsdal SX/cross contrast. This test is auxiliary to the "
                "SN Refsdal closure test."
            ),
        },
    }
