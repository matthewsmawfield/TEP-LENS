...
Sweeps the TEP coupling parameter alpha across 0.001 to 0.150, computing:
  - TEP closure residual R_TEP(alpha) for all 5 SN Refsdal loops
  ...
  - Falsification threshold alpha_falsify: alpha at which R_TEP = 1 day (sensitive limit)

Produces three publication figures:
  Figure 1: R_TEP vs alpha for all 5 loops (fan plot)
...
def main():
    ...
    ax1.axvline(-0.05, color="grey", lw=1.0, ls=":", zorder=1,
                label=r"$\alpha = -0.05$ (calibrated)")
    ...
    ax2.axvline(-0.05, color="grey",    lw=1.0, ls=":", label=r"$\alpha = -0.05$")
    ...
    ax4.axvline(-0.05, color="grey", lw=1.0, ls=":", label=r"$\alpha=-0.05$")
    ...
    results = {
        ...
        "key_finding": (
            "SNR = |R_TEP| / sigma_R is exactly alpha-independent for all loops. "
            "R_TEP and sigma_R both scale linearly with alpha, so their ratio is "
            "a pure geometric constant determined by the delay ratios and magnification "
            "contrasts alone. Inner-cross loops (S1-S2-S3, S1-S2-S4) have "
            "intrinsic SNR < 3 at any alpha. SX loops have intrinsic SNR = 60-78 "
            "at any alpha > 0. The detection question is purely geometric."
        ),
