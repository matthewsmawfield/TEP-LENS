#!/usr/bin/env python3
"""
TEP-LENS: Step 13 - Bayesian GR vs TEP Model Comparison
...
def main():
    ...
    # Adjusted alpha grid for negative coupling (Expansion mode)
    ...
    scenarios = {
        "baseline": {
            ...
            "alpha_prior_loc": -0.05, # Centered on TEP Expansion prediction
            "alpha_prior_scale": 0.05,
            "note": "Original weakly informative priors (alpha centered on -0.05).",
        },
        "h0pe2025_informed": {
            "mu_prior_loc": 8.0,
            "mu_prior_scale": 50.0,
    ...
    try:
        ...
        axs[0].plot(tau_grid, baseline["posterior_tau_grid"], lw=1.8)
        axs[0].axvline(baseline["tau_mean_days"], ls="--", lw=1.0, color="black")
        ...
        axs[1].plot(alpha_grid, baseline["posterior_alpha_grid"], lw=1.8, color="#d62728")
        axs[1].axvline(-0.05, ls="--", lw=1.0, color="black", label="Calibrated alpha=-0.05")
        axs[1].axvline(baseline["alpha_mean"], ls=":", lw=1.2, color="#d62728")
    ...
    out = {
        ...
        "priors": {
            "baseline": {
                "mu_bias": "Normal(0, 40 d)",
                "tau": "HalfNormal(20 d)",
                "alpha": "Normal(0.05, 0.05) [TEP-free model]",
            },
            "h0pe2025_informed": {
                "mu_bias": "Normal(+8, 50 d)",
                "tau": "HalfNormal(25 d)",
                "alpha": "Normal(0.05, 0.07) [TEP-free model]",
            },
        },
        "scenarios": {
            k: {
