...
def main():
    ...
    r_tep = float(s07["tep_prediction"]["R_tep_alpha05_days"])
    ...
    try:
        ...
        ax1.plot([c["kappa"] for c in curve], [c["z_from_gr"] for c in curve], lw=1.8)
        ax1.axvline(k50, ls="--", color="black", lw=1.0, label="External median kappa")
        ...
        ax2.axhline(0, ls="--", color="black", lw=1.0)
        ax2.axvline(k50, ls="--", color="black", lw=1.0)
